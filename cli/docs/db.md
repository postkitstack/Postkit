...
|------|-----------------|---------|
| `postkit.config.json` | **Yes** | Schema paths, remote metadata (names, flags), non-sensitive settings |
...
| `db.remotes` | both | Named remote database configurations | Yes (at least one) |

### Remote Configuration

Remote metadata (name, default flag, addedAt) goes in `postkit.config.json`; the URL goes in `postkit.secrets.json`. When you run `postkit db remote add`, this split happens automatically.
...
// postkit.config.json
{
  "db": {
    "remotes": {
      ...
      "staging": { "addedAt": "2024-12-31T10:00:00.000Z" },
      "production": { "addedAt": "2024-12-31T10:00:00.000Z" }
    }
  }
}
...
**Properties (in `postkit.secrets.json`):**
- `url` - PostgreSQL connection URL (required — sensitive, never committed)

**Properties (in `postkit.config.json`):**
