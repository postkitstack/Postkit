...
import { generateGrants, getGrantsSQL, applyGrants } from '../services/grant-generator.js';
...
export async function grantsCommand(options: GrantsOptions): Promise<void> {
  ...
  try {
    ...
    if (options.apply) {
      ...
      if (options.target === 'remote') {
        ...
        if (!session || !session.active) {
          logger.error('No active session. Cannot apply grants to local database.');
          logger.info('Run "npm run migr start" first or use --target=remote.');
          process.exit(1);
        }
    ...
    logger.info('  - Run "npm run migr grants --apply" to apply to local clone');
    logger.info('  - Run "npm run migr grants --apply --target=remote" to apply to remote');
  } catch (error) {
