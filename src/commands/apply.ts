...
export async function applyCommand(options: CommandOptions): Promise<void> {
  ...
  try {
    ...
    if (session.pendingChanges.applied) {
      ...
      logger.info('Run "npm run migr commit <description>" to create a migration file.');
      logger.info('Or run "npm run migr plan" again if you made more changes.');
      return;
    }
    ...
    if (!localConnected) {
      ...
      logger.info('The local clone may have been removed. Run "npm run migr start" again.');
      process.exit(1);
    }
    ...
    logger.info('  - Run "npm run migr commit <description>" to create migration and apply to remote');
    logger.info('  - Or run "npm run migr abort" to cancel if something is wrong');
  } catch (error) {
