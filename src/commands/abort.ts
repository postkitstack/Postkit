...
export async function abortCommand(options: CommandOptions): Promise<void> {
  ...
  try {
    ...
    logger.info(`  Plan generated: ${session.pendingChanges.planned ? 'Yes' : 'No'}`);
    logger.info(`  Applied to local: ${session.pendingChanges.applied ? 'Yes' : 'No'}`);
      ...
      const { confirm } = await inquirer.prompt([
        {
          type: 'confirm',
          name: 'confirm',
          message: 'Are you sure you want to abort this session? All changes will be lost.',
          default: false,
        },
      ]);
