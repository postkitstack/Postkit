...
export async function startCommand(options: CommandOptions): Promise<void> {
  ...
  try {
    ...
    if (await hasActiveSession()) {
      ...
      logger.info('Run "migr abort" to cancel it or "migr status" to see details.');
      process.exit(1);
    }
    ...
    if (!dbmateInstalled) {
      ...
      logger.info('Install with: brew install dbmate (macOS) or go install github.com/amacneil/dbmate@latest');
      process.exit(1);
    }
    ...
    logger.debug(`Remote DB: ${maskConnectionUrl(config.remoteDbUrl)}`, options.verbose);
    logger.debug(`Local DB: ${maskConnectionUrl(config.localDbUrl)}`, options.verbose);
    ...
    if (!remoteConnected) {
      ...
      logger.error('Could not connect to the remote database. Check your REMOTE_DATABASE_URL.');
      process.exit(1);
    }
    ...
    if (!options.dryRun) {
      const session = await createSession(config.remoteDbUrl, config.localDbUrl);
    ...
    logger.info('  2. Run "npm run migr plan" to see changes');
    logger.info('  3. Run "npm run migr apply" to test on local clone');
    logger.info('  4. Run "npm run migr commit <description>" when ready');
  } catch (error) {
